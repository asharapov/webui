...
public final class RefType extends Type {
    ...
    public RefType(final Class cls) {
        Class cl = cls;
        int ai = 0;
        while (cl.isArray()) {
     ...
     */
    public RefType(final String qname, final Type... args) {
        this(qname);
        if (args.length>0) {
            this.args = new ArrayList<Type>(args.length);
            for (Type tp : args) {
                if (tp != null) {
                    tp.setParent(this);
                    this.args.add(tp);
    ...
    public List<Type> getTypeArguments() {
        return args !=null ? args : Collections.<Type>emptyList();
    }

    /**
     * Добавляет описание параметра типа.
     * @param typeArg новый параметр.
     * @return  то же значение что и в аргументе.
     */
    public Type addTypeArgument(final Type typeArg) {
        if (args ==null)
            args = new ArrayList<Type>(2);
